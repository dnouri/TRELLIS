"""
API key authentication for the Modal TRELLIS service.
...
def _cli_list_keys(args: Any, keys_path: str) -> None:
    ...
    for key, info in keys.items():
        ...
        print(f"{mask_api_key(key):20} {info.get('name', 'unnamed'):15} {status:8} usage: {usage}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Manage API keys for TRELLIS Modal service"
    )
    parser.add_argument(
        "--keys-file",
    ...
    add_parser.add_argument("--name", required=True, help="Human-readable name for the key")
